...
\begin{description}
\item[continous bag-of-words] (CBOW)\index{CBOW@\emph{CBOW}} Se considera la palabra central y un contexto de $n$ palabras anteriores y posteriores, que se tratará como un conjunto (bolsa de palabras), sin tener en cuenta su orden de aparición en la frase.
\item[skip-gram] \index{skip-gram@\emph{skip-gram}} En este modelo, se considera la palabra central y un contexto de $n$ palabras anteriores y posteriores, con posibilidad de saltar $k$ palabras, teniendo además en cuenta su orden de aparición.
\end{description}
