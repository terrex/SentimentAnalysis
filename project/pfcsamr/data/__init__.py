...
try:
    ...
    if not os.path.exists(file):
        urlretrieve("https://www.kaggle.com/c/sentiment-analysis-on-movie-reviews/download/sampleSubmission.csv",
            filename=file)

    file = os.path.join(here, 'data', 'test.tsv')
    filezip = file + ".zip"
    if not os.path.exists(file):
        urlretrieve("https://www.kaggle.com/c/sentiment-analysis-on-movie-reviews/download/test.tsv.zip",
            filename=filezip)
    ...
    filezip = file + ".zip"
    if not os.path.exists(file):
        urlretrieve("https://www.kaggle.com/c/sentiment-analysis-on-movie-reviews/download/train.tsv.zip",
            filename=filezip)
        zipfile.ZipFile(filezip).extractall(os.path.dirname(file))
